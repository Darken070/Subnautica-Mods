...
|Config.TryGet(ref _x, " - ", "Size", "x") 
|Config.TryGet(ref _y, " - ", "Size", "y") 
...
{ 
  _x = 1; 
  Config[" - ", "Size", "x"] = _x; 
  Utilites.Logger.Logger.Error("Size of  can't be less or equal to 0 X was set to 1", Utilites.Logger.LogType.Custom | Utilites.Logger.LogType.Console); 
  configChanged = true; 
} 
if (_y <= 0) 
{ 
  _y = 1; 
  Config[" - ", "Size", "y"] = _y; 
  Utilites.Logger.Logger.Error("Size of  can't be less or equal to 0 Y was set to 1", Utilites.Logger.LogType.Custom | Utilites.Logger.LogType.Console); 
  configChanged = true; 
} 
if (_xMIUraninite <= 0) 
{ 
...
var techType = TechTypePatcher.AddTechType("", "", ""); 
...
var sprite = assetBundle.LoadAsset<Sprite>(""); 
...
CraftDataPatcher.customTechData.Add(TechType., techDataB); 
CraftTreePatcher.customCraftNodes.Add("Resources/AdvancedMaterials/", techType); 
CraftTreePatcher.customCraftNodes.Add("Resources/BasicMaterials/", TechType.); 
CraftDataPatcher.customItemSizes[key: techType] = new Vector2int(x: _x, y: _y); 
